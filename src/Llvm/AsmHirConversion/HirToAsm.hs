...
data ReaderData = ReaderData { mp :: (M.Map (I.GlobalId (), H.Label) A.LabelId)
                             , funname :: I.GlobalId ()
                             }
type Rm = Reader ReaderData

withFunName :: I.GlobalId () -> Rm a -> Rm a
...
instance Conversion (I.TlDefine () ()) (Rm A.TlDefine) where
...
convert_Comdat :: I.Comdat () -> A.Comdat      
...
convertNode :: I.Node () () e x -> Rm ([A.Block], M.Map A.LabelId A.Block, Maybe Pblock)
...
graphToBlocks :: H.Graph (I.Node () ()) H.C H.C -> Rm ([A.Block], M.Map A.LabelId A.Block)
graphToBlocks g = do { (bl, bs, Nothing) <- H.foldGraphNodes convertNode g (return ([], M.empty, Nothing))
                     ; return (reverse bl, bs)
                     }

toplevel2Ast :: I.Toplevel () () -> Rm A.Toplevel
...
hirToAsm ::  DataLayoutMetrics dlm => M.Map (I.GlobalId (), H.Label) A.LabelId -> I.SpecializedModule dlm () () -> A.Module
hirToAsm iLm (I.SpecializedModule dlm (I.Module ts)) = 
  let (A.Module tl) = runReader (Md.liftM A.Module (mapM toplevel2Ast ts)) (ReaderData iLm (I.GlobalIdNum 0))
      ls = toLayoutSpec dlm
      tt = toTriple dlm
