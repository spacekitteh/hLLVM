{-# LANGUAGE CPP, TypeFamilies, TupleSections, FlexibleInstances, MultiParamTypeClasses,  UndecidableInstances  #-}
...
instance MonadReader r m => MonadReader r (UniqueMonadT m) where
  ask = UMT $ \us -> liftM (,us) ask
  local f m = UMT $ \us -> local f (unUMT m us)

instance MonadState s m => MonadState s (UniqueMonadT m) where
  get = UMT $ \us -> liftM (,us) get
  put s = UMT $ \us -> liftM (,us) (put s)
